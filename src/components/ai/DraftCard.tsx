    ...
    <Card className={`${hasError ? "border-red-500" : ""}`}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <Label htmlFor={`front-${draft.id}`}>Przód (pytanie)</Label>
          <Textarea
            id={`front-${draft.id}`}
            value={draft.front}
            onChange={(e) => onUpdate("front", e.target.value)}
            placeholder="Wpisz pytanie..."
            className={`min-h-[80px] ${isFrontInvalid ? "border-red-500 focus-visible:ring-red-500" : ""}`}
          />
          <div className="flex items-center justify-between text-sm">
            <span className={`${isFrontTooLong ? "text-red-500 font-medium" : "text-muted-foreground"}`}>
              {frontLength} / {frontConstraints.max}
            </span>
            {isFrontEmpty && <span className="text-red-500 text-xs">Pole wymagane</span>}
            {isFrontTooLong && (
              <span className="text-red-500 text-xs">Przekroczono limit o {frontLength - frontConstraints.max}</span>
            )}
          </div>
        </div>
        ...
        {/* Back (Answer) */}
        <div className="space-y-2">
          <Label htmlFor={`back-${draft.id}`}>Tył (odpowiedź)</Label>
          <Textarea
            id={`back-${draft.id}`}
            value={draft.back}
            onChange={(e) => onUpdate("back", e.target.value)}
            placeholder="Wpisz odpowiedź..."
            className={`min-h-[120px] ${isBackInvalid ? "border-red-500 focus-visible:ring-red-500" : ""}`}
          />
          <div className="flex items-center justify-between text-sm">
            <span className={`${isBackTooLong ? "text-red-500 font-medium" : "text-muted-foreground"}`}>
              {backLength} / {backConstraints.max}
            </span>
            {isBackEmpty && <span className="text-red-500 text-xs">Pole wymagane</span>}
            {isBackTooLong && (
              <span className="text-red-500 text-xs">Przekroczono limit o {backLength - backConstraints.max}</span>
            )}
          </div>
        </div>
      </CardContent>
